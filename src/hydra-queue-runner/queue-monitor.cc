...
void State::queueMonitorLoop(Connection &conn) {
  ...
  while (!quit) {
    ...
    if (buildOne && buildOneDone)
      quit = true;

    /* Sleep until we get notification from the database about an
       event. */
    if (done && !quit) {
      conn.await_notification(5 * 50, 0);
      nrQueueWakeups++;
    } else
      conn.get_notifs();

    if (auto lowestId = buildsAdded.get()) {
      lastBuildId = std::min(lastBuildId,
                             static_cast<unsigned>(std::stoul(*lowestId) - 1));
...
bool State::getQueuedBuilds(Connection &conn, ref<Store> destStore,
...

  {
    ...
    auto res = txn.exec_params(
        ...
        "where builds.id > $1 and finished = 0 order by globalPriority desc, "
        "builds.id",
        lastBuildId);
  ...
  createBuild = [&](Build::ptr build) {
    ...
    printMsg(lvlTalkative, "loading build %1% (%2%)", build->id,
             build->fullJobName());
    nrAdded++;
    ...
    if (!localStore->isValidPath(build->drvPath)) {
      ...
      if (!build->finishedInDB) {
        ...
        pqxx::work txn(conn);
        txn.exec_params0(
            "update Builds set finished = 1, buildStatus = $2, startTime = $3, "
            "stopTime = $3 where id = $1 and finished = 0",
            build->id, (int)bsAborted, time(0));
        txn.commit();
    ...
    try {
      step = createStep(destStore, conn, build, build->drvPath, build, 0,
                        finishedDrvs, newSteps, newRunnable);
    } catch (PreviousFailure &ex) {

      /* Some step previously failed, so mark the build as
         failed right away. */
      if (!buildOneDone && build->id == buildOne)
        buildOneDone = true;
      ...
      if (!build->finishedInDB) {
        ...
        auto res = txn.exec_params1(
            "select max(build) from BuildSteps where drvPath = $1 and "
            "startTime != 0 and stopTime != 0 and status = 1",
            localStore->printStorePath(ex.step->drvPath));
        if (!res[0].is_null())
          propagatedFrom = res[0].as<BuildID>();

        if (!propagatedFrom) {
          for (auto &[outputName, optOutputPath] :
               destStore->queryPartialDerivationOutputMap(ex.step->drvPath,
                                                          &*localStore)) {
            constexpr std::string_view common =
                "select max(s.build) from BuildSteps s join BuildStepOutputs o "
                "on s.build = o.build where startTime != 0 and stopTime != 0 "
                "and status = 1";
        ...
        createBuildStep(txn, 0, build->id, ex.step, "", bsCachedFailure, "",
                        propagatedFrom);
        txn.exec_params(
            "update Builds set finished = 1, buildStatus = $2, startTime = $3, "
            "stopTime = $3, isCachedBuild = 1, notificationPendingSince = $3 "
    ...
    for (auto &r : newSteps) {
      ...
      if (i == newBuildsByPath.end())
        continue;
      auto j = newBuildsByID.find(i->second);
      if (j == newBuildsByID.end())
        continue;
    ...
    if (!step) {
      ...
      for (auto &i :
           destStore->queryDerivationOutputMap(build->drvPath, &*localStore))
        addRoot(i.second);
    ...
    printMsg(lvlChatty, "added build %1% (top-level step %2%, %3% new steps)",
             build->id, localStore->printStorePath(step->drvPath),
             newSteps.size());
  };
  ...
  for (auto id : newIDs) {
    ...
    /* Add the new runnable build steps to ‘runnable’ and wake up
       the builder threads. */
    printMsg(lvlChatty, "got %1% new runnable steps from %2% new builds",
             newRunnable.size(), nrAdded);
    for (auto &r : newRunnable)
      makeRunnable(r);

    if (buildOne && newRunnable.size() == 0)
      buildOneDone = true;
  ...
  lastBuildId =
      newBuildsByID.empty() ? newLastBuildId : newBuildsByID.begin()->first - 1;
...
void Build::propagatePriorities() {
  ...
  visitDependencies(
      [&](const Step::ptr &step) {
        ...
        step_->highestGlobalPriority =
            std::max(step_->highestGlobalPriority, globalPriority);
        step_->highestLocalPriority =
            std::max(step_->highestLocalPriority, localPriority);
...
void State::processQueueChange(Connection &conn) {
  ...
  std::map<BuildID, int> currentIds;
  {
    pqxx::work txn(conn);
    auto res =
        txn.exec("select id, globalPriority from Builds where finished = 0");
    for (auto const &row : res)
...
Step::ptr State::createStep(ref<Store> destStore, Connection &conn,
                            Build::ptr build, const StorePath &drvPath,
  ...
  printMsg(lvlDebug, "considering derivation ‘%1%’",
  ...
  {
    StringSet features = step->requiredSystemFeatures =
        step->parsedDrv->getRequiredSystemFeatures();
  ...
  for (auto &[outputName, maybeOutputPath] :
       destStore->queryPartialDerivationOutputMap(drvPath, &*localStore)) {
  ...
  if (!missing.empty()) {
    ...
    for (auto &[i, pathOpt] : missing) {
      ...
      if (/* localStore != destStore && */ !pathOpt &&
    ...
    if (missing.size() == avail) {
      ...
      for (auto &[i, pathOpt] : missing) {
        ...
        try {
          ...
          copyClosure(*localStore, *destStore, StorePathSet{path}, NoRepair,
                      CheckSigs, NoSubstitute);
...

          {
            auto mc = startDbUpdate();
            pqxx::work txn(conn);
            createSubstitutionStep(txn, startTime, stopTime, build, drvPath,
                                   *(step->drv), "out", path);
            txn.commit();
          }
          ...
          printError("while copying/substituting output ‘%s’ of ‘%s’: %s",
                     localStore->printStorePath(path),
                     localStore->printStorePath(drvPath), e.what());
          valid = false;
          break;
        }
      }
  ...
  /* No, we need to build. */
  printMsg(lvlDebug, "creating build step ‘%1%’",
           localStore->printStorePath(drvPath));

  /* Create steps for the dependencies. */
  for (auto &i : step->drv->inputDrvs.map) {
    auto dep = createStep(destStore, conn, build, i.first, 0, step,
                          finishedDrvs, newSteps, newRunnable);
    if (dep) {
      auto step_(step->state.lock());
...
Jobset::ptr State::createJobset(pqxx::work &txn, const std::string &projectName,
                                const std::string &jobsetName,
                                const JobsetID jobsetID) {
  ...
  auto res = txn.exec_params1(
      "select schedulingShares from Jobsets where id = $1", jobsetID);
  if (res.empty())
    throw Error("missing jobset - can't happen");
  ...
  auto res2 = txn.exec_params(
      "select s.startTime, s.stopTime from BuildSteps s join Builds b on build "
      "= id "
      "where s.startTime is not null and s.stopTime > $1 and jobset_id = $2",
      time(0) - Jobset::schedulingWindow * 10, jobsetID);
...
void State::processJobsetSharesChange(Connection &conn) {
  ...
  for (auto const &row : res) {
    auto jobsets_(jobsets.lock());
    auto i = jobsets_->find(std::make_pair(row["project"].as<std::string>(),
...
BuildOutput State::getBuildOutputCached(Connection &conn,
                                        nix::ref<nix::Store> destStore,
                                        const nix::StorePath &drvPath) {
  auto derivationOutputs =
      destStore->queryDerivationOutputMap(drvPath, &*localStore);

  {
    pqxx::work txn(conn);

    for (auto &[name, output] : derivationOutputs) {
      auto r = txn.exec_params("select id, buildStatus, releaseName, "
                               "closureSize, size from Builds b "
                               "join BuildOutputs o on b.id = o.build "
                               "where finished = 1 and (buildStatus = 0 or "
                               "buildStatus = 6) and path = $1",
      ...
      auto products = txn.exec_params(
          ...
          "from BuildProducts where build = $1 order by productnr",
          id);
      ...
      for (auto row : products) {
        ...
        if (!row[3].is_null())
          product.sha256hash =
              Hash::parseAny(row[3].as<std::string>(), HashAlgorithm::SHA256);
        if (!row[4].is_null())
      ...
      auto metrics = txn.exec_params(
          "select name, unit, value from BuildMetrics where build = $1", id);
